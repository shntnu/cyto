// prettier-ignore
const en = {
  translation: {
    'Cancel': 'रद्द करें',
    'Categories': 'सारे वर्ग',
    'Create category': 'वर्ग बनाएँ',
    'Create new classifier': 'नया classifier बनाएं',
    'Delete category': 'वर्ग हटाएँ',
    'Delete images': 'चित्रों को हटाएँ',
    'Description': 'विवरण',
    'Edit category': 'वर्ग का बदलाव करें',
    'Help': 'सहायता',
    'Hide other categories': 'बाकि वर्गों को छिपाएं',
    'Hide sidebar': 'साइडबार छिपाएं',
    'Import images': 'चित्रों को आयात करें',
    'Logo': 'Logo',
    'Model': 'Model',
    'Open classifier': 'Classifier खोलें',
    'Open example classifier': 'Example classifier खोलें',
    'Open weights': 'Weights खोलें',
    'Open': 'खोलें',
    'Run classifier': 'Run classifier',
    'Save annotations and predictions': 'Annotations तथा predictions सहेजें',
    'Save classifier': 'Classifier सहेजें',
    'Save weights': 'Weights सहेजें',
    'Save': 'सहेजें',
    'Search images': 'चित्र खोजें',
    'Send feedback': 'Feedback भेजें',
    'Settings': 'सेटिंग्स',
    'Show sidebar': 'साइडबार दिखाएं',
    'Unknown': 'Unknown'
  }
};

export default en;
